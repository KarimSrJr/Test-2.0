...
(async function(){
  ...
  const fmtDateShort = (iso) => new Date(iso).toLocaleDateString([], {month:'short', day:'2-digit'});
  ...
  if(page === 'home') {
    ...
    document.getElementById('kpiScorers').textContent = data.scorers.length;
    ...
    data.fixtures.forEach(f => {
      ...
      card.className = 'kpi';
      card.innerHTML = `<div class="num" style="min-width:70px">${fmtDateShort(f.date)}</div>
